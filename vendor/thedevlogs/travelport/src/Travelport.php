...
class Travelport
{
    const BASE_URI = 'https://apac.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/AirService';
    ...
    public function checkAirAvailability($routesArr)
    {
		...
		//var_dump($routesArr);exit();
		//var_dump($target_branch);exit();
		$postdata = array();
    ...
    private function _apiCall($method, $postdata = [], $xml, $extraData = []){
        ...
        if(!empty($postdata))
        {
            ...
            $file = "001-" . config('travelport.provider') . $xml . "_Req.xml"; // file name to save the request xml for test only(if you want to save the request/response)
